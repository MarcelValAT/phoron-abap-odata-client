...
CLASS zcl_attachment_v2_client IMPLEMENTATION.
  ...
  METHOD get_fi_doc_attachments.
    ...
    TRY.
        " Parameter-Struktur befüllen
        DATA ls_params TYPE zcl_scm_odata_crud_attm=>tys_parameters_3.
        ls_params-business_object_type_name = 'BKPF'.
        ls_params-linked_sapobject_key = lv_key.

        " FunctionImport-Resource holen und Parameter VOR create_request setzen
        DATA(lo_func_resource) = mo_client_proxy->create_resource_for_function_import( zcl_scm_odata_crud_attm=>gcs_function_import-get_all_originals ).
        lo_func_resource->set_parameter( is_parameter = ls_params ).

        " Request erstellen, ausführen und Ergebnis holen
        DATA(lo_response) = lo_func_resource->create_request( )->execute( ).
  ...
  METHOD download_attachment.
    ...
    TRY.
        ...
        DATA(lo_resp) = lo_http_client->execute( if_web_http_client=>get ).
        ...
        rv_content = lo_resp->get_binary_data( ).
